...
function getarraytime(bat,lessonurl){
    ...
    $.each(bat, function(i,data){
      let id = data.id;
      let url = lessonurl+id;
      let lesson = getData(url).data;
...
function displayReservation(instructor,timeData){
...
$(function(){
  !localStorage.loginsess ? window.location.href = 'index.html' : '';
  ...
  instructor === null ? $('.alert-danger').removeClass('d-none'):$('.alert-danger').addClass('d-none')
  displayTimeSlot(timeData);
  displayReservation(instructor,timeData);
...
$("#reservationdate").change(function(){
  ...
  console.log(searchdate,instructor)
